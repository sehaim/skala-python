...
SHOW_BCRYPT_COST_NOTE = True  # bcrypt 비용 관련 안내 로그 출력
...
def hash_password(plain_password: str, rounds: int = 12) -> bytes:
    """
    비밀번호를 bcrypt로 해시한다.
    - rounds(costi factor)가 높을수록 느려지며 공격 비용 증가(실습은 12 권장)
    """
...
def login_attempt(ip: str, password_input: str, password_hash: bytes) -> bool:
    """
    IP별 실패 횟수 기록 + 지수 백오프 차단을 적용한 로그인 시도 함수.
    콘솔에 결과를 출력한다.
    """
    now = time.time()
    ...
    if remain > 0:
        print(f"[BLOCK] ip={ip}  남은 대기={remain:.2f}s  (fail_count={state.fail_count})")
        return False
...
def demo_scenario():
    ...
    """
    print("\n==============================")
    print("bcrypt + IP별 실패기록 + 지수 백오프 로그인 방어 실습")
    print("==============================\n")
    ...
    if SHOW_BCRYPT_COST_NOTE:
        print("※ 참고: bcrypt rounds(비용)가 높을수록 검증이 느려지지만 공격 비용도 커집니다.\n")
    ...
    for i in range(6):
        login_attempt(attacker_ip, password_input="wrong_password", password_hash=password_hash)
        time.sleep(0.4)  # 일부러 짧게 재시도 -> 차단 로그가 보이도록

    # 정상 사용자: 다른 IP는 영향을 받지 않음
    print("\n---- 정상 사용자 시도 (다른 IP는 별도 카운트) ----")
    login_attempt(normal_ip, password_input="correct_password", password_hash=password_hash)
    ...
    login_attempt(attacker_ip, password_input="correct_password", password_hash=password_hash)

    # 차단이 풀릴 때까지 기다렸다가 성공 시나리오
